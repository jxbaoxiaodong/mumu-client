...
const CHINA_REGIONS = {
    ...
    "海南省": {
        ...
        "省直辖县级": ["五指山市", "琼海市", "文昌市", "万宁市", "东方市", "定安县", "屯昌县", "澄迈县", "临高县", "白沙黎族自治县", "昌江黎族自治县", "乐东黎族自治县", "陵水黎族自治县", "保亭黎族苗族自治县", "琼中黎族苗族自治县"]
    },
    "四川省": {
...
function getCitiesByProvince(province) {
    if (!CHINA_REGIONS[province]) return [];
...
function getDistrictsByCity(province, city) {
    if (!CHINA_REGIONS[province] || !CHINA_REGIONS[province][city]) return [];
    return CHINA_REGIONS[province][city];
}

/**
 * 初始化省市区三级联动选择器
 * @param {Object} options - 配置选项
 * @param {string} options.provinceSelect - 省份选择器ID
 * @param {string} options.citySelect - 城市选择器ID
 * @param {string} options.districtSelect - 区县选择器ID (可选)
 * @param {string} options.defaultProvince - 默认省份
 * @param {string} options.defaultCity - 默认城市
 * @param {string} options.defaultDistrict - 默认区县
 * @param {Function} options.onChange - 选择变化回调函数
 */
function initRegionSelector(options) {
    const provinceSelect = document.getElementById(options.provinceSelect);
    const citySelect = document.getElementById(options.citySelect);
    const districtSelect = options.districtSelect ? document.getElementById(options.districtSelect) : null;

    if (!provinceSelect || !citySelect) {
        console.error('Province and city select elements are required');
        return;
    }
    ...
    function fillProvinces() {
        ...
        provinces.forEach(province => {
            ...
            if (province === options.defaultProvince) {
    ...
    function fillDistricts(province, city) {
        ...
        districts.forEach(district => {
            ...
            if (district === options.defaultDistrict) {
                option.selected = true;
            }
    ...
    provinceSelect.addEventListener('change', function() {
        ...
        fillCities(province);
        fillDistricts(province, '');
        if (options.onChange) {
            options.onChange({ province: province, city: '', district: '' });
        }
    });
    ...
    citySelect.addEventListener('change', function() {
        ...
        fillDistricts(province, city);
        if (options.onChange) {
            options.onChange({ province: province, city: city, district: '' });
        }
    });
    ...
    if (options.defaultProvince) {
        fillCities(options.defaultProvince);
        if (options.defaultCity) {
            fillDistricts(options.defaultProvince, options.defaultCity);
        }
    }
}
